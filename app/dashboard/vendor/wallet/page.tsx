...
export default function VendorWallet() {
  ...
  const loadWalletData = (userId: string) => {
    // 1. Load basic wallet
    const storedWallet = localStorage.getItem(`af_vendor_wallet_${userId}`)
    let bal = 250.0
    let life = 1540.0
    let ledg: LedgerItem[] = []
    ...
    if (storedWallet) {
      ...
      bal = parsed.balance ?? 250.0
      life = parsed.lifetime ?? 1540.0
      ledg = parsed.ledgers ?? []
    } else {
      ledg = [
        { id: "l1", type: "credit", amount: 150.00, desc: "Order payment cleared ORD-X7B", date: new Date(Date.now() - 86400000).toISOString() },
        { id: "l2", type: "withdrawal", amount: 100.00, desc: "Withdrawal to M-Pesa", date: new Date(Date.now() - 172800000).toISOString() },
        { id: "l3", type: "credit", amount: 200.00, desc: "Order payment cleared ORD-M9F", date: new Date(Date.now() - 259200000).toISOString() }
      ]
      localStorage.setItem(
        `af_vendor_wallet_${userId}`,
    ...
    // 2. Load active orders to calculate escrows (pending, confirmed, dispatched total prices)
    const storedOrders = localStorage.getItem(`af_vendor_orders_${userId}`)
    if (storedOrders) {
      const ordersList: Order[] = JSON.parse(storedOrders)
      const escrows = ordersList
        .filter((o) => ["pending", "confirmed", "dispatched"].includes(o.status))
  ...
  const handleWithdraw = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      try {
        ...
        if (session?.user) {
          localStorage.setItem(
            `af_vendor_wallet_${session.user.id}`,
    ...
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold text-white flex items-center gap-3">
          ...
          <span>Vendor Wallet & Payouts</span>
        </h1>
        <p className="text-muted-foreground mt-1">
          Monitor cleared available earnings, track funds currently held in buyer escrows, and request instant payouts.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="glass p-6 rounded-xl border border-primary/20 relative overflow-hidden">
          <span className="text-xs text-primary font-bold uppercase tracking-wider block">Available Balance</span>
          <h3 className="text-3xl font-black text-white mt-1">${balance.toFixed(2)}</h3>
          <p className="text-xs text-muted-foreground mt-1">Cleared funds ready to withdraw.</p>
        ...
        <div className="glass p-6 rounded-xl border border-amber-500/20 relative overflow-hidden">
          <span className="text-xs text-amber-500 font-bold uppercase tracking-wider block">Escrow Holdings</span>
          <h3 className="text-3xl font-black text-white mt-1">${escrowBalance.toFixed(2)}</h3>
          <p className="text-xs text-muted-foreground mt-1">Locked in active dispatches.</p>
          <div className="absolute top-4 right-4 h-9 w-9 rounded-lg bg-amber-500/10 flex items-center justify-center font-bold text-amber-500">
            Esc
          </div>
        </div>

        <div className="glass p-6 rounded-xl border border-border/40 relative overflow-hidden">
          <span className="text-xs text-muted-foreground font-bold uppercase tracking-wider block">Lifetime Sales</span>
          <h3 className="text-3xl font-black text-white mt-1">${lifetime.toFixed(2)}</h3>
          <p className="text-xs text-muted-foreground mt-1">Total revenue generated on platform.</p>
          <div className="absolute top-4 right-4 h-9 w-9 rounded-lg bg-slate-800 flex items-center justify-center font-bold text-white">
            $$
          </div>
        </div>
      </div>
